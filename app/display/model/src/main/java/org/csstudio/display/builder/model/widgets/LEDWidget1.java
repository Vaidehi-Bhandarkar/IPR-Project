...
public class LEDWidget1 extends BaseLEDWidget
{
    /** Widget descriptor */
    public static final WidgetDescriptor WIDGET_DESCRIPTOR =
        new WidgetDescriptor("led1", WidgetCategory.MONITOR,
            "LED1",
            "/icons/led.png",
            "LED that represents on/off",
            Arrays.asList("org.csstudio.opibuilder.widgets.LED1"))
    {
        @Override
        public Widget createWidget()
        {
            return new LEDWidget1();
        }
    };

    /** Custom configurator to read legacy *.opi files */
    private static class LED1Configurator extends WidgetConfigurator
    {
        public LED1Configurator(final Version xml_version)
        {
            super(xml_version);
        }
                ...
                throws Exception
        {
            ...
            final LEDWidget1 led1 = (LEDWidget1) widget;
            handle_legacy_LED(model_reader, led1, xml_version, xml);
            ...
            XMLUtil.getChildBoolean(xml, "show_boolean_label").ifPresent(show ->
            {
                if (! show)
                {
                    led1.propOffLabel().setValue("");
                    led1.propOnLabel().setValue("");
                }
            });
            return true;
        }
    ...
    public WidgetConfigurator getConfigurator(final Version persisted_version)
            throws Exception
    {
        return new LED1Configurator(persisted_version);
    }
}
