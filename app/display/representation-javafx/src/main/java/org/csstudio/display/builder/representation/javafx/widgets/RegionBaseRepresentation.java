//project/phoebus-master/app/display/representation-javafx/src/main/java/org/csstudio/display/builder/representation/javafx/widgets/RegionBaseRepresentation.java
...
//NEW
import static org.csstudio.display.builder.model.properties.CommonWidgetProperties.runtimePropPV1Value;
...
//NEW
import org.csstudio.display.builder.model.widgets.PV1Widget;
...
abstract public class RegionBaseRepresentation<JFX extends Region, MW extends VisibleWidget> extends JFXBaseRepresentation<JFX, MW>
{
    ...
    //NEW
    private volatile WidgetProperty<VType> value_prop1 = null;
    private volatile WidgetProperty<Boolean> alarm_sensitive_border_prop1 = null;
    private volatile WidgetProperty<WidgetColor> border_color_prop1 = null;
    private volatile WidgetProperty<Integer> border_width_prop1 = null;
    ...
    protected void registerListeners()
    {
        ...
        if (cust_col.isPresent()  &&  cust_wid.isPresent())
        {
            ...
            //NEW
            border_color_prop1 = cust_col.get();
            border_width_prop1 = cust_wid.get();
            border_color_prop1.addUntypedPropertyListener(customBorderChangedListener);
            border_width_prop1.addUntypedPropertyListener(customBorderChangedListener);
        ...
        if (alarm_sens.isPresent()  &&  value.isPresent())
        {
            value_prop1 = value.get();
            alarm_sensitive_border_prop1 = alarm_sens.get();
            // runtimeValue should be a VType,
            // but some widgets may allow other data types (Table),
            // so use Object and then check for VType
            value_prop1.addUntypedPropertyListener(connectionOrValueChangedListener);
        }
    ...
    protected void unregisterListeners()
    {
        ...
        //NEW
        if (border_color_prop1 != null)
            border_color_prop1.removePropertyListener(customBorderChangedListener);
        if (border_width_prop1 != null)
            border_width_prop1.removePropertyListener(customBorderChangedListener);

        if (!toolkit.isEditMode())
        {
            if (value_prop1 != null)
                value_prop1.removePropertyListener(connectionOrValueChangedListener);
    ...
    private void hookMiddleButtonCopy(final MouseEvent event)
    {
        ...
        //NEW
        final String pv1_name = ((PV1Widget)model_widget).propPV1Name().getValue();
        ...
        content.putString(pv1_name);
        Clipboard.getSystemClipboard().setContent(content1);
        ...
        //NEW
        if (Toolkit.getDefaultToolkit().getSystemSelection() != null)
            Toolkit.getDefaultToolkit().getSystemSelection().setContents(new StringSelection(pv1_name), null);
    ...
    private void connectionOrValueChanged(final WidgetProperty<?> property, final Object old_value, final Object new_value)
    {
        ...
        {   // Reflect severity of primary PV's value
            ...
            //NEW
            if (value_prop1 != null  && alarm_sensitive_border_prop1.getValue())
            {
                final Object value = value_prop1.getValue();
                final Alarm alarm = Alarm.alarmOf(value);
                if (alarm != null  &&  alarm.getSeverity() != AlarmSeverity.NONE)
                    // Have alarm info
                    severity = alarm.getSeverity();
